...
async function ocrPlate(buffer, filename) {
  ...
  if (!token) {
    console.log('OCR: PLATE_RECOGNIZER_TOKEN not configured, skipping OCR');
    return null;
  }
  try {
    const form = new FormData();
    form.append('upload', new Blob([buffer]), filename || 'photo.jpg');
    form.append('regions', 'de'); // bias toward German plate formats
    ...
    if (!res.ok) {
      console.error('OCR error:', res.status, await res.text().catch(()=>''));
      return null;
    }
    const data = await res.json();
